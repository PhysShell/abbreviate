...
## Тест набора (`test.html`)

`http://localhost:8000/test.html` — замер скорости: показывается целевой текст,
отсчёт 3-2-1, таймер стартует с первого символа и сам останавливается при
совпадении ввода с целью. Два режима для сравнения — **обычно** (движок выключен,
просто textarea) и **с подсказками**. Метрики: время, зн/мин, сл/мин, число
нажатий (ввод + тапы по подсказкам), сколько подсказок принято и какая доля из
них была верхней в списке. Та же `WasmEngine`, всё офлайн.
