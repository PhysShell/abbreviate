...
- `⌫` — в правом конце нижнего буквенного ряда (подальше от системной
...
- **`⇄`** — переключение кириллической раскладки между **полной ЙЦУКЕН**
  (по умолчанию — для тех, кто привык) и **согласной**: основные ряды содержат
  только согласные (на своих местах из ЙЦУКЕН, гласные убраны) — ровно то, под
  что эта клавиатура и сделана. Сверху — **тонкий ряд гласных**, который
  сворачивается в узкую полоску тогглом `▴`/`▾`. Выбор раскладки и видимость
  ряда гласных запоминаются между сессиями (`SharedPreferences`);
...
**Сессионный recency-кэш (per-app).** Оболочка кормит движок закоммиченными
словами (`noteWord`): принятую подсказку — в `SuggestionController.accept`,
а слово, набранное без подсказки, — на сепараторе (пробел/`↵`) через
`noteCommitted`. Так недавно употреблённое слово всплывает выше, а слова вне
лексикона (`синхрофазотрон`) вообще становятся достижимы. Скоуп — **per-app**:
в `onStartInputView` оболочка сравнивает `EditorInfo.packageName` и при смене
приложения зовёт `resetSession`, чтобы кэш не перетекал между приложениями. IME
видит только пакет, не конкретный чат внутри него, поэтому это per-app, а не
per-conversation (разбор — `docs/RESEARCH-RECENCY-CACHE.md` §5). Кэш эфемерный,
в синк не идёт, новых разрешений не требует.
