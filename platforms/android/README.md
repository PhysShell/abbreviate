...
| `mask/ru.txt`             | `mask.txt`      | список мата для маскирования (§5.2)     |
| `tone/ru.tsv`             | `tone.tsv`      | знаковые маркеры тона/регистра (§5.1)   |

`UniffiSuggestionPort.fromData(lexicon, lm, shortcuts, mask, tone)` поднимает движок; в
...
словами (`noteWord`): явно выбранную подсказку (тап по чипу) — сразу; слово,
набранное без подсказки, — на сепараторе (пробел/`↵`) через `noteCommitted`.
Авто-акцепт по «умному пробелу» откладывается до закрытия окна отмены
(`keepAutoAccept`): мгновенный `⌫` его откатывает, и тогда отклонённое слово в
кэш не попадает. Так недавно употреблённое слово всплывает выше, а слова вне
...
**Маскирование мата + тон/регистр (§5.1/§5.2).** `EngineLoader` подгружает
`mask.txt` (список мата) и `tone.tsv` (знаковые маркеры тона) в движок, но
маскирование по умолчанию **выключено** — оболочка не переписывает набранное,
пока пользователь не попросит. Настройки хранятся в `SharedPreferences`
(`mask_profanity`, `mask_when_polite`, обе `false` по умолчанию) и
прокидываются в движок через `SuggestionController.setMasking` в `onCreate` и
на каждом `onStartInputView` (`applyMaskingPrefs`). Когда включено, движок
предлагает **маск-двойник рядом** (`долбоёб → дол@#&б`), не убирая оригинал;
`mask_when_polite` дополнительно гейтит это по тону окна — цензурим в вежливом
окне, среди своих (где мат ласковый) нет. Тон считается из того же потока
`noteWord`, без chat-id и новых разрешений. (UI-тумблера в этой спайке пока нет —
ключи выставляются программно/через `adb`; см. роадмап.)
