...
6. ~~Проводка тона/масок в FFI/оболочку~~ — **сделано**: UniFFI отдаёт
   `load_mask_list`/`load_tone_markers`/`set_masking`/`register` (+ enum
   `Register`), движок — рантайм-сеттер `set_masking`. Android грузит `mask.txt`/
   `tone.tsv` в ассеты, `SuggestionController.setMasking` биндится к
   `SharedPreferences` (по умолчанию выкл), тон-гейт работает от живого окна
   `noteWord`. UI-тумблера пока нет (ключи через `adb`).
7. **Ранжирующий** register-сигнал (§5.1): следующий шаг. Нужны размеченные по
   регистру кандидаты (сленг ↔ нейтраль) и офлайн-бенч без регрессий (см. §8).
