...
5. Register-сигнал (§5.1) + тон-гейт для маскирования: следующий шаг. Тон
   считается из окна `note_word` (без chat-id и новых разрешений); открыт способ
   мерить тон (§8). Туда же — проводка `EngineConfig.mask`/профиля в FFI и
   оболочку.
