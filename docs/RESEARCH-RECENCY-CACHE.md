...
recency как ещё один аддитивный сигнал с весом `w.recency`, подбираемым на
...
## 4. Часть 2 — OOV-ретривал (вариант B: параллельный путь)
...
## 5. Часть 3 — граница «этого разговора»: per-app
...
1. **Recency-срез в генеративном бенчмарке.** Расширить `abbrev gen`: генерить
   мини-«разговоры» — слово вводится дважды, второй раз сокращённо, между
   вхождениями шум из N других слов. Метрика: top-1/top-3 на втором вхождении с
   кэшем и без. Burstiness предсказывает заметный подъём, особенно для редких по
   корпусу слов.
2. **OOV-достижимость.** Доля слов вне лексикона, попавших в top-3 после одного
   `note` (без recency — недостижимы вовсе, это контроль).
3. **Без регрессий.** Приёмочный `data/bench/basic.tsv` держит 100%; на общем
   генеративном наборе (без повторов) recency нейтрален — `w.recency` не должен
...
* **Заглавные/имена собственные.** «Синхрофазотрон» vs «Синхрофазотрон»: хранить
  нормализованно (как сейчас), форму отображения восстанавливать на вставке.
...
* **Register-сигнал (§5.1): как мерить тон окна.** Чем оценивать «вежливость»
  недавнего потока — долей слов из сленгового слоя, отдельным маркер-списком,
  или лёгким классификатором? И как калибровать вес, чтобы тон сдвигал баланс
  «формы ↔ сленг», но не ломал обычное ранжирование на нейтральном тексте
  (тот же запрет регрессий, что и для `w.recency`).
* **Маскирование мата (§5.2): подмена или кандидат.** Предлагать `дол@#&б`
  кандидатом в ленте (внутри модели «не трогаем») или всё же делать осторожную
  автозамену с откатом? И насколько агрессивно бороться с обфускацией (`д0лб`),
  не раздувая список и не плодя ложные срабатывания.
...
1. Часть 1: `recency.rs`, сигнал, вес, юнит-тесты, recency-срез в `abbrev gen` —
   honest quick win с измеримым эффектом.
2. Часть 2 (B): `trait` источника OOV-кандидатов + параллельный merge в
   `suggest`; синтетический низкий приор по аналогии с именами.
3. Проводка оболочки: `note_word`/`reset` через FFI/WASM; per-app ключ и момент
   `reset` на стороне Android-оболочки.
