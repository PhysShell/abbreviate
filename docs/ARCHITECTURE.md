...
| delete-индекс (SymSpell) | скелет минус один символ | опечатки в согласных: `кмртер → компьютер` — без него слово не достаётся вовсе; отключаем (`typo_tolerance`), т.к. стоит памяти |

Нормализация всюду одна: lowercase + `ё→е`; скелет дополнительно отбрасывает
гласные и `ь/ъ`. Все бакеты отсортированы по частоте на этапе построения,
поэтому любые кэпы выборки оставляют самые частотные формы, а не первые по
алфавиту. `with_prefix` — top-k по частоте через ограниченную кучу с
**бюджетом сканирования** (`cap·16` записей): в пределах бюджета результат
точный, дальше широкий диапазон отбрасывается — worst-case работа на нажатие
остаётся ограниченной.
`PrefixMap` — единственное место для замены на FST/DAWG, если профилирование
на устройстве этого потребует.
...
4. typo-tolerance: delete-варианты скелета против delete-индекса (точное
   покрытие правок скелета при дистанции ≤ 1: замена, лишний/пропавший
   согласный, транспозиция).
...
* Контекст — `trait ContextModel`. Реализована компактная **биграмная LM**
  (`ngram::BigramModel`, артефакт `#abbrev-lm v1` от `lexicon-builder
  bigrams`): положительный PMI последнего слова контекста, невиданные пары
  нейтральны. На контекстном бенчмарке даёт +13пп top-1 (BENCHMARKS.md).
  Следующая ступень — квантованный нейрореранкер за тем же trait.
* Персонализация — `UserHistory`: на каждую пару «скелет ввода → форма»
  раздельные счётчики `confirmed`/`reverted` (тап ≠ успех — см.
  [DIAGNOSTICS.md](DIAGNOSTICS.md)). Приор — *чистая* улика
  `ln(1+confirmed) − ln(1+reverted)` + слабая глобальная популярность формы; она
  уходит в минус, когда откаты перевешивают, поэтому многократно отклонённая
...
* **Морфологический сигнал** (`w_morph`, `morph::compatibility`) — согласование
  ...
  предшествующим предлогом (`в работе`, `для работы`, `про работу`). Сигнал
  мягкий — совпадение поднимает, несовпадение нейтрально (предлоги с несколькими
  падежами не штрафуют). На падежно-неоднозначном бенчмарке +23пп top-1 без
  регрессий (BENCHMARKS.md). Комплементарен LM: та ловит лексику, морфология —
  грамматическое управление, и работает на парах, которых нет в корпусе.
...
* **жизненный цикл коррекции и диагностика** — `CorrectionSession`, детекция
  отмены через `InputConnection`, приватный логгинг и debug-bundle. Это зона
  оболочки (ядро sans-IO часов и persistence не несёт); спека —
...
flowchart LR
    ...
    NM["Natasha-имена<br/>(mawo-nlp-data, MIT)"] --> LB
...
некоммерческая, с атрибуцией); скачивается с dict.ruslang.ru.

`lexicon-builder expand lexicon.tsv ru-hold-groups.tsv -o …` досыпает
недостающие словоформы парадигмы для **уже присутствующих** лемм (новых слов
не добавляет): ~50% склоняемых лемм есть в субтитровом списке одной формой,
поэтому набранное окончание не дотягивается до нужной (`длгсрчная` не находит
`долгосрочная`). Синтет-форма получает частоту `round(max_present × ratio)`,
зажатую строго ниже самой редкой реальной формы леммы — так ретривал+ранжир
держат частотные формы сверху (`арми`→армии, не армиями), а skeleton/suffix
поднимают точное совпадение окончания. Идемпотентно; новых лемм — ноль; рост
артефакта ~×2.5. Импортёр OpenCorpora-XML — следующий шаг. Артефакт лексикона
версионируется и собирается в CI, на устройстве никогда не строится.
...
артефактов, **не** зависимость устройства: Android читает готовый компактный
артефакт, без Python в рантайме.

**Длинный хвост имён.** `scripts/names.py` — сиблинг той же mawo-pymorphy3-цепочки:
берёт списки имён/фамилий/отчеств (Natasha-словари из mawo-nlp-data, MIT) и
склоняет по падежам (фамилии — ещё и в семейном множественном: Ивановы),
отдавая обычные строки лексикона `form<TAB>lemma<TAB>freq<TAB>tags`. Эмитятся
только формы, которые pymorphy реально метит как `Name/Surn/Patr` (без
NOUN-фолбэка — иначе незнакомые токены давали брак: угаданные сущ., омографы
`Ивановец`/`Иваново`-Geox); из ~190k входных имён OpenCorpora распознаёт ~24k →
~176k форм. Имена не имеют корпусной частоты — у всех форм плоский низкий приор
(`--freq`), так что реальное слово всегда выигрывает омограф (`роман`-сущ. над
`Роман`-именем), но имя достижимо. Граммемы НКРЯ из mawo-nlp-data при этом
**не берутся**: условий редистрибуции у них нет, частоты остаются build-time-only.
Артефакт версионируется **gzip** (`data/lexicons/ru-names.tsv.gz`, ~1.2 МБ против
~12 МБ сырьём); build разжимает его (`gunzip`) перед мёрджем с `ru-50k.tsv` —
так `lexicon-builder` остаётся без внешних зависимостей. Образец схемы (plain) —
`data/lexicons/ru-names.sample.tsv`; сам `.gz` собирается в CI
(`.github/workflows/build-names.yml`, `workflow_dispatch`): тянет релиз
mawo-nlp-data, прогоняет `names.py`, гзипует и пушит в ветку `bot/ru-names`
(Actions не может создавать PR под настройкой орг — PR открывает человек).
Шаг не работает в офлайн-sandbox (нет PyPI/доступа к релизу), поэтому в GH.
Мёрдж в рантайм — **opt-in**: `Lexicon::extend_from_tsv_str` догружает имена в
базовый лексикон (CLI: повторяемый `--extra-lexicon`), без дедупа — омограф
`Роман`/`роман` остаётся двумя записями, выбор за ранкером. Имена не форсятся
по умолчанию: +176k записей ≈ +2.5× лексикон и заметная латентность, так что
платформа решает, грузить ли их.
...
Третий контур — **property-based / fuzz-тесты** (`crates/abbrev-core/tests/properties.rs`,
proptest как dev-dependency — в рантайм не попадает, ADR-0002 не нарушает). Они
фиксируют не отдельные кейсы, а инварианты на тысячах сгенерированных входов:
парсеры (`Lexicon`/`Paradigms`) не паникуют на произвольных байтах; `skeleton`
не содержит гласных/знаков; `normalize` идемпотентен; `suggest` не паникует и
уважает `limit`; и — главное — **`UserHistory::merge` коммутативен/ассоциативен
с нейтралью** (CRDT-закон, на котором держится модель синка «блоб + merge»).

## 9. Дорожная карта

1. **Сейчас (каркас)** — ядро с пятью слоями, CLI, бенчмарк, FFI/WASM-обвязка. ✅
2. **Лексикон** — частично сделано: боевой лексикон 48k форм лемматизирован
   pymorphy3/OpenCorpora (18 636 лемм, `scripts/lemmatize.py`). Дальше: полный
   импортёр OpenCorpora (парадигмы, граммемы), частоты НКРЯ, бинарный формат,
   бенчмарк на 10⁵+ форм.
...
4. ~~**Контекст**~~ — сделано: биграмная LM за `ContextModel`
   ...
   Измерение recency-лифта — `abbrev bench --recency` (cold vs warm). Part 3
   (per-app-скоуп) сделана в Android IME: `noteWord`/`resetSession` по
   `EditorInfo.packageName`. Дальше — нейрореранкер и (из ресёрча) register-сигнал.
5. ~~**Морфология**~~ — сделано (первый слой): граммемы OpenCorpora в 4-й
   ...
   группировка hold по падежу, умная фильтрация «морфологической свалки».
...
7. ~~**Устойчивость к опечаткам**~~ — сделано: SymSpell delete-индекс поверх
   скелетов поднял срез `typo` с 39.9% до 68.9% top-3 (BENCHMARKS.md).
