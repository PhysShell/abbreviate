...
```text
score = w_skel·skeleton_match + w_suf·suffix_compat − w_edit·edit_distance
      + w_freq·ln(1+ipm) + w_ctx·context_lm + w_user·user_prior
```
...
suggest(&self, input, &Context, limit) -> Vec<Suggestion>  // короткий ввод ⇒ пусто
...
Сид-набор `data/bench/basic.tsv` покрывает выпадение гласных, `ь`, флективные
цели, опечатки и транспозиции; его нужно наращивать программной генерацией
(удаление гласных/букв + соседние клавиши) из OpenCorpora/НКРЯ-лексики.
...
5. **Морфология** — парадигмы из OpenCorpora: сигнал `w_morph`, группировка
   подсказок по леммам, умное «подержать — формы».
6. **Продвинутое** — нейрореранкер (квантованный), фразовые сокращения,
   iOS/десктоп/web-оболочки по мере готовности ядра.
