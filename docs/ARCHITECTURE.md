...
берёт списки имён/фамилий/отчеств (Natasha-словари из mawo-nlp-data, ~113k, MIT)
и склоняет каждое имя по падежам (фамилии — ещё и в семейном множественном:
Ивановы), отдавая обычные строки лексикона `form<TAB>lemma<TAB>freq<TAB>tags`
(`data/lexicons/ru-names.tsv`, мёрджится с `ru-50k.tsv`). Имена не имеют
корпусной частоты — у всех форм плоский низкий приор (`--freq`), так что реальное
слово всегда выигрывает омограф (`роман`-сущ. над `Роман`-именем), но имя
достижимо. Граммемы НКРЯ из mawo-nlp-data при этом **не берутся**: условий
редистрибуции у них нет, частоты остаются build-time-only (см. выше).
Образец схемы — `data/lexicons/ru-names.sample.tsv`; сам артефакт собирается в
CI (`.github/workflows/build-names.yml`, `workflow_dispatch`): тянет релиз
mawo-nlp-data, прогоняет `names.py` и открывает PR с `ru-names.tsv` — этот шаг
не работает в офлайн-sandbox (нет PyPI/доступа к релизу), поэтому вынесен в GH.
